...
- [x] 1.14 `feat(raft): restart from storage` — node rebuilds from HardState + log; tests
- [x] 1.15 `feat(raft): event stream` — structured events (`BecameLeader`, `VoteGranted`, `EntryCommitted`, ...) for the UI
...
- [x] 2.10 `feat(sim): history + rewind` — record user actions; rewind = rebuild from seed and replay (exact, since runs are deterministic); step back/forward
...
- [x] 3.3 `test(wasm): bridge smoke test` — node-based test of the compiled WASM (runs in CI)
...
- [x] 4.7 `feat(fe): node actions` — click node: crash / restart / pause; client write to node
- [x] 4.8 `test(fe): store + ClusterRing tests` — against MockClient (written with each step); this step fills the gaps: animation loop, toast timeout, coverage report
